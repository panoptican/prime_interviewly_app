...
/*
 Schedule GET factory that retrieves and formats a schedule
 */
app.factory('scheduleGet', ['$http', function($http) {

    return {

        newSchedule: function(eventParam) {},

        savedSchedule: function(eventParam) {},

        // function takes in event ID and UI Grid arrays with time slot info populated
        formatSchedule: function (eventParam, gridCols, gridData) {

            return $http.get('api/event/getSchedule?_id=' + eventParam).then(function (response) {

                // empty variables for storing data
                var schedDetails = {};

                // store schedule ID in case it is saved
                schedDetails.id = response.data._id;
